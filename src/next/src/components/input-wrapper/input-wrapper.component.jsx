...
const addMessage = (parsedData, messagesStore, currentRoom) => {
  const [_type, messageType, payload] = parsedData;
  const { roomId, senderId, message, createdAt } = payload;
  ...
  if (id !== senderId && roomId === currentRoom.roomId) {
    messagesStore?.add({
      type: messageType,
      sender: "bot",
      props: { text: message || "Error", createdAt },
    });
  }
};

export default function InputWrapper(props) {
  const { Original, messagesStore, scroll, botControls } = props;
